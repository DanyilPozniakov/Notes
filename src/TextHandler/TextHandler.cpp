...
void TextHandler::setTextDocument(QObject* textEditObject)
{
    ...
    if (quickDoc)
    {
        ...
        m_textOptions.setTabStopDistance(36); //Microsoft format
...
void TextHandler::formatCode()
{
    QString code = m_textDocument->toPlainText();

    QProcess process;
    QStringList arguments;

    arguments << "-style=Microsoft";

    process.start("clang-format", arguments);

    if (!process.waitForStarted())
    {
        qWarning() << "Error process";
    }
    ...
    {
        qDebug() << "clang-format не завершился корректно";
        return;
    }

    QByteArray formatedCode = process.readAllStandardOutput();
    QByteArray errorOutputs = process.readAllStandardError();

    if (errorOutputs.isEmpty())
    {
        m_textDocument->setPlainText(QString::fromUtf8(formatedCode));
    }
    else
    {
        qWarning() << "Formatetion Error";
    }
}
